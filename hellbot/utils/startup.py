...
from dolybot import LOGS
from dolybot.config import Config
from dolybot.helpers.int_str import make_int
from dolybot.sql.gvar_sql import addgvar, gvarstat


# Creates the logger group on first deploy and adds the helper bot
async def logger_id(client):
    desc = "A Bot Logger Group For Dolybot. DO NOT LEAVE THIS GROUP!!"
    ...
    try:
        grp = await client(functions.channels.CreateChannelRequest(title="Dolybot Logger", about=desc, megagroup=True))
...
<b><i>»» <u><a href='https://t.me/BTS_CHAT_ZONE'> ＤＯＬＹＢＯＴ 🇮🇳 </a></u> ««</i></b>
"""
    await client.send_file(
        ...
        caption=text,
        parse_mode="HTML",
        buttons=[[Button.url("DolyBot Network", "https://t.me/BTS_CHAT_ZONE")]],
    )


# Joins the dolybot chat and channel from all clients
async def join_it(client):
    if client:
        try:
            await client(functions.channels.JoinChannelRequest("@BTS_CHAT_ZONE"))
